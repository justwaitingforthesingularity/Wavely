...
import { getAlbumDetails, AlbumDetails } from "@/services/pipedApi";
    ...
    getAlbumDetails(id)
      .then((result) => {
        if (!cancelled) {
          setData(result);
          setLoading(false);
          if (result.album.thumbnail) {
            extractColor(result.album.thumbnail);
          }
        }
      })
      .catch((err) => {
  ...
  if (loading) {
    return (
      <div className="animate-fadeIn flex flex-col items-center justify-center min-h-screen gap-3">
        ...
        <p className="text-[13px] text-white/30">Loading album...</p>
      </div>
    );
  }
  ...
  if (error || !data) {
    return (
      <div className="animate-fadeIn flex flex-col items-center justify-center min-h-screen gap-3 px-5">
        <p className="text-[15px] text-white/50">{error || "Album not found"}</p>
        <button
          onClick={() => router.back()}
          ...
          <div className="flex gap-5">
            ...
            <div className="flex-1 min-w-0 flex flex-col justify-end">
              ...
              <p className="text-[12px] text-white/25 mt-0.5">
                {album.year ? `${album.year} \u00B7 ` : ""}{songQueue.length} {songQueue.length === 1 ? "song" : "songs"}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="px-5">
          ...
          {songQueue.length === 0 && (
            <div className="flex flex-col items-center justify-center py-16 gap-3">
              <p className="text-[14px] text-white/40">No songs in this album</p>
            </div>
          )}
        </div>
      </div>
